...
func VaultBootFetchSnippet(addrVar, roleIDVar, secretIDVar, kvPath, key, outVar string) string {
	...
	w("  VAULT_LOGIN_RESP=$(curl -fsS -X POST \"$VAULT_ADDR/v1/auth/approle/login\" \\")
	...
	w("VAULT_READ_RESP=\"\"")
	w("for attempt in 1 2 3 4 5; do")
	w("  VAULT_READ_RESP=$(curl -fsS -H \"X-Vault-Token: $VAULT_TOKEN\" \"$VAULT_ADDR/v1/secret/data/%s\" 2>/dev/null || true)", kvPath)
