...
func RenderBackendDOUserData(spec BackendBootstrapSpec) (string, error) {
	...
	w("(crontab -l -u root 2>/dev/null | grep -v 'health-probe.sh'; echo \"* * * * * /home/main/health-probe.sh\") | sudo crontab -u root -")
	w("")
	...
	w("echo \"Pulling backend native binary (version='$PYX_VERSION') from DO Spaces s3://%s/%s ...\"", backendSpacesBucket, backendSpacesKey)
	w("if [ -n \"$PYX_VERSION\" ] && /usr/local/bin/aws s3 cp \"s3://%s/%s-$PYX_VERSION\" /home/main/pyxcloud --endpoint-url \"$SPACES_ENDPOINT\" --region %s; then", backendSpacesBucket, backendSpacesKey, backendSpacesRegion)
	w("  echo \"Pulled versioned native binary %s-$PYX_VERSION.\"", backendSpacesKey)
	w("else")
	w("  echo \"Versioned key missing/unset; falling back to the stable '%s' key.\"", backendSpacesKey)
	w("  /usr/local/bin/aws s3 cp \"s3://%s/%s\" /home/main/pyxcloud --endpoint-url \"$SPACES_ENDPOINT\" --region %s", backendSpacesBucket, backendSpacesKey, backendSpacesRegion)
	w("fi")
