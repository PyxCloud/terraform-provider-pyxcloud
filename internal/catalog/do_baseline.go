...
func AssembleDOBaseline(ctx context.Context, cat Catalog, in AssembleInput, secrets DOBaselineSecrets, opts DOBaselineOptions) ([]string, error) {
  ...
  droplet_template {
	...
	//    origin nginx serves its own cert) with a TCP:443 health check. The legacy
	//    single "edge-lb" (backend, https-passthrough + http:8080 healthcheck) was
	//    renamed to "lb-backend" and converted to this uniform TCP config live; the
	//    catalog now models the reconciled shape.
	//
