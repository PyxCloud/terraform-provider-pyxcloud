...
func TestMacroUnknownProviderRejected(t *testing.T) {
	t.Parallel()
	cat := MustEmbedded()
	if _, err := TranslateCache(ctx(), cat, CacheSpec{Region: "Frankfurt", Provider: "oracle", MemoryGB: 1}); err == nil {
		t.Error("cache: unknown provider should error")
	}
	if _, err := TranslateServerless(ctx(), cat, ServerlessSpec{Region: "Frankfurt", Provider: "oracle"}); err == nil {
		t.Error("serverless: unknown provider should error")
	}
	if _, err := TranslateKubernetes(ctx(), cat, K8sSpec{Region: "Frankfurt", Provider: "oracle", NodeCPU: 2, NodeRAM: 4}); err == nil {
