...
func TestManagedDatabaseValidation(t *testing.T) {
	...
	cases := []struct {
		...
		{"unknown provider", ManagedDatabaseSpec{Region: "Frankfurt", Provider: "azure", CPU: 2, RAM: 4}},
...
func TestRenderManagedDatabaseUnsupportedProvider(t *testing.T) {
	t.Parallel()
	if _, err := RenderManagedDatabaseHCL(ManagedDatabasePlan{Provider: "azure"}); err == nil {
