...
func main() {
	...
	provider := flag.String("provider", "", "target provider: aws | gcp | digitalocean | ovh (wave-2)")
	...
	if *fixturePath == "" || *provider == "" {
		fmt.Fprintln(os.Stderr, "usage: pyxnet-render -fixture f.json -provider aws|gcp|digitalocean|ovh [-component network|security-group]")
		os.Exit(2)
	}
